...
static void firmare_update_c33(void)
{
    ...
    for(i=0;i<READREG_RETRY_TIMES;i++)
    {
        mdelay(100);      //  needs about 400~600ms in fact ,20*100ms is enough
        reg_data = drvDB_ReadReg ( 0x3C, 0xE4 );
        if(0x2F43 == reg_data)
        break;
    }
...
*/
static void Msg21XX_proximity_enable(bool enableflag)//  1:Enable or 0:Disable  CTP Proximity function\A1\EA?
{
